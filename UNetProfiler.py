...
class UNetProfiler():
    ...
    def __call__(self, name, shape, autoPrint=False):
        ...
        if autoPrint: print(f"Event name: {name} - Time: {eventTime:.0f}s, Memory: {memory:.1f}GB, Shape: {shape}")

    def printEvents(self):
        for i in range(len(self.eventList)):
            name, eventTime, memory, shape = self.eventList[i]
            if i == 0:
                print(f"Event name: {name} - Time: {eventTime:.0f}s, Memory: {memory:.1f}GB")
            else:
                prevEventTime, prevMemory = self.eventList[i - 1][1:3]
                timeDiff = eventTime - prevEventTime
                memoryDiff = memory - prevMemory
                print(f"Event name: {name} - Time: {timeDiff:.0f}s, Memory: {memoryDiff:.1f}GB, Shape: {shape}")